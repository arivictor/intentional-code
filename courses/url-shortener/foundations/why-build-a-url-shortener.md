...
A URL shortener is the best first backend you can build, and its small surface area is exactly why. 
...
We've decided *what* we're building and *why*. Next we make the one architectural decision the entire course rests on: cleanly separating the three things this service does such as generating codes, storing links, and speaking HTTP, so that  each can change without disrupting the others. We'll put [Separation of Concerns](/go/philosophy/separation-of-concerns) into practice.
