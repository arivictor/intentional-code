...
intent: "Process a stream of data through a series of stages, each running in its own goroutine and connected to the next by a channel."
idiomSummary: "Each stage is a function that receives an input channel and returns an output channel; goroutines are launched inside the stage function, not at the call site."
...
- Stages are so fast that channel overhead dominates. Measure before adding goroutines to a tight loop.
