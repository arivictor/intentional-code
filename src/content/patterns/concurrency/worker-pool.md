...
The jobs channel buffer size is a real design decision. An unbuffered channel means the sender blocks until a worker is free, providing natural backpressure. A large buffer absorbs bursts but can accumulate jobs faster than workers process them, growing memory without bound if the sender is much faster than the workers. A buffer of one to two times the worker count is a reasonable starting point.

The pool's error model is also a choice: the simple version above collects errors into the results channel and lets the consumer decide what to do. The [Errgroup](/go/patterns/concurrency/errgroup) pattern cancels the whole pool on the first error, which is appropriate when any failure makes the rest of the work pointless.
