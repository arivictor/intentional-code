...
intent: "Distribute work from a single channel across multiple goroutines, then merge their results back into one channel."
idiomSummary: "Fan out with a loop spawning goroutines each reading the same input channel; fan in with a merger goroutine that selects across output channels and closes the merged channel when all workers are done."
