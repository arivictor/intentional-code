...
intent: "Process data through a sequence of independent, composable transformation steps where each step reads from input, transforms it, and writes to output."
...
The pattern appears under different names: data pipeline, processing pipeline, ETL pipeline. The core property is the same: independent stages, composable in any order, each testable in isolation.
