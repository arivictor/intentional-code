...
> In most Python codebases, a type-switch is preferred over Visitor. It's simpler, more readable, and Python's object model tells you when you've missed a case (with exhaustive switch linters). Use Visitor only when you truly need the open/closed principle for operations — e.g., a compiler or interpreter where new analysis passes are added frequently but the AST node types are stable.
