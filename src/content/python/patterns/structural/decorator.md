...
idiomSummary: "Wrap callables or objects to add behavior without modifying the wrapped implementation."
...
Decorator wraps an object to add behavior, keeping the same interface. In Python, this pattern is everywhere — it's how HTTP middleware works. Any function that takes an interface and returns the same interface, adding behavior in between, is a decorator.

The canonical Go example is `http.Handler` middleware: a function that takes a handler, returns a new handler that logs, authenticates, compresses, or rate-limits, and then calls the original. It's the [Open/Closed Principle](/python/philosophy/solid) made concrete — each concern is added without touching the code it wraps.
...
def handle_order(w, r):
    ...
    w.Write(list[byte("order processed"))
```
...
def order_handler(w, r):
    w.Write(list[byte("order processed"))

def health_handler(w, r):
    w.Write(list[byte("ok"))
...
- Deep decorator stacks (5+ layers) make debugging difficult. Consider whether a [Chain of Responsibility](/python/patterns/behavioral/chain-of-responsibility) would be clearer.
...
- Standard Python style for HTTP middleware — instantly recognizable.
