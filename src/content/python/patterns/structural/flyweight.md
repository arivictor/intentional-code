...
idiomSummary: "Share immutable state and keep per-use state outside the reusable object."
...
class Tree:
    ...
    Texture list[byte  // Large texture data — identical for same species
...
type_cache = map[string]*TreeType{}

def get_tree_type(name, texture, color):
    key = name
